...
def get_image_files(source_folder):
    try:
        ...
        logging.error(f"Failed to fetch image files from {source_folder}: {e}")
        return []
...
def create_output_folder(source_folder, format="WEBP"):
    output_folder = f"{source_folder}_{format.lower()}"
    try:
...
def process_file(file_path, output_folder, quality, stats, format='WEBP'):
    try:
        output_file_path = os.path.join(output_folder, os.path.relpath(file_path, source_folder))
        output_file_dir = os.path.dirname(output_file_path)
        os.makedirs(output_file_dir, exist_ok=True)
        output_file_path = os.path.splitext(output_file_path)[0] + f'.{format.lower()}'

        original_size = os.path.getsize(file_path)
        stats['total_original_size'] += original_size

        with Image.open(file_path) as img:
            img.convert("RGB").save(output_file_path, format, quality=quality)
...
def convert_images_to_webp(source_folder, quality=80, threads=64, format="WEBP"):
    ...
    output_folder = create_output_folder(source_folder, format)
    if output_folder is None:
        return

    # Use ThreadPoolExecutor to perform conversions in parallel, improving performance.
    with ThreadPoolExecutor(max_workers=threads) as executor:
        list(tqdm(executor.map(lambda file_path: process_file(file_path, output_folder, quality, stats, format), file_list), unit="file"))
...
if __name__ == "__main__":
    ...
    convert_images_to_webp(source_folder, format="WEBP")